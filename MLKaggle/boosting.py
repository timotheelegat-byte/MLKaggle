...
def add_geometry_features(gdf: gpd.GeoDataFrame) -> pd.DataFrame:
    ...
    # check final numeric safety
    total_replaced = 0

    for col in out.columns:
        # compter inf et -inf
        inf_count = np.isinf(out[col]).sum()
        nan_count = out[col].isna().sum()
    
        total_replaced += inf_count + nan_count
    
        # remplacer inf par nan, puis nan par 0
        out[col] = out[col].replace([np.inf, -np.inf], np.nan).fillna(0)

    print(f"Total de valeurs inf/nan remplacées par 0 : {total_replaced}")
    return out
...
# HistGradientBoosting in sklearn needs dense input -> convert AFTER preprocessing
def to_dense(X):
    return X.toarray() if hasattr(X, "toarray") else X

# ---------- 6) Model + tuning (HistGradientBoosting only) ----------
f1_macro = make_scorer(f1_score, average="macro")

# Toggle for faster iterative tuning vs. final run
FAST_MODE = False
REFINE_STAGE = False  # run a second, narrower search around the best fast params
...
print("Tuning HistGradientBoosting with HalvingGridSearchCV...")
...
if FAST_MODE and REFINE_STAGE:
    # Build a narrower grid around the best fast parameters
    best = search.best_params_

    def unique_sorted(vals):
        return sorted(set(vals))

    lr = best.get("clf__learning_rate", 0.1)
    lr_grid = unique_sorted([max(0.01, lr / 2), lr, min(0.2, lr * 1.5)])

    md = best.get("clf__max_depth", None)
    if md is None:
        md_grid = [None, 6, 10]
    else:
        md_grid = unique_sorted([max(2, md - 2), md, md + 2])

    mln = best.get("clf__max_leaf_nodes", 31)
    mln_grid = unique_sorted([max(15, mln // 2), mln, int(mln * 1.5)])

    msl = best.get("clf__min_samples_leaf", 20)
    msl_grid = unique_sorted([max(10, msl // 2), msl, int(msl * 1.5)])

    mi = best.get("clf__max_iter", 200)
    mi_grid = unique_sorted([max(100, mi // 2), mi, int(mi * 1.5)])

    refine_grid = {
        "clf__learning_rate": lr_grid,
        "clf__max_depth": md_grid,
        "clf__max_leaf_nodes": mln_grid,
        "clf__min_samples_leaf": msl_grid,
        "clf__max_iter": mi_grid,
    }
