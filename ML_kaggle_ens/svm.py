...
# ---------- 6) Model + tuning (SVM only) ----------
# Use LinearSVC (fast, sparse-friendly). If you need probabilities, we calibrate it.
...
# CalibratedClassifierCV gives predict_proba; it adds cost but is often worth it.
# You can remove calibration if you don't need probabilities.
...
if FAST_MODE:
    param_grid = {
        "clf__estimator__C": [0.05, 0.1, 0.3, 1.0],
    }
else:
    param_grid = {
        "clf__estimator__C": [0.05, 0.1, 0.3, 1.0, 3.0, 10.0],
    }
...
print("Tuning (Linear) SVM with HalvingGridSearchCV...")
...
if FAST_MODE and REFINE_STAGE:
    ...
    best_c = best.get("clf__estimator__C", 1.0)
    refine_grid = {
        "clf__estimator__C": sorted({max(0.01, best_c / 2), best_c, best_c * 2}),
    }
...
sub.to_csv("sample_submission_svm_1.csv", index=False)
print("Wrote sample_submission_svm_1.csv")